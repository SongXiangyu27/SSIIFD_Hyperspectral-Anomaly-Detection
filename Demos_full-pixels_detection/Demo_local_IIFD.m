...
%
% This is a demo of using IIFD for anomaly detection in spectral-domain with an ERS-based segmentation preprocessing
%
...
filename = 'TexasCoast';   % SanDiegoI  SanDiegoII  TexasCoast  Gulfport                           
load (filename)
...
tic
[~,~,score_LIIF, ~, ~] = Local_IIFD(Data, Data, map_mx1, NumTree, num_rows, num_cols);
toc

%  show detection map
LIIFD_show = reshape(score_LIIF, num_rows, num_cols);
figure('name','Local_IIFD');imshow(LIIFD_show,[]);
% figure('name','Local_IIFD');
% image(LIIFD_show,'Parent',axes,'CDataMapping','scaled')
...
%% result_show
results = [score_LIIF, score_KIF, r_PTA, r_CRD, r_RX_norm'];
legend_3DROC = {'Local_IIFD', 'KIFD', 'PTA', 'CRD', 'RXD'};
